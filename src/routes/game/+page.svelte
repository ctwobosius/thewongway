<script lang="ts">
  import PythonTutorial from '../python/+page.svelte'
  const sections_ = {
  ...
  var held_directions = [0, 0, 0, 0]; // State of which arrow keys we are holding down
  var sprinting_factor = 1;

  let dpad_left, dpad_right, dpad_up, dpad_down;
  $: elements = [dpad_left, dpad_right, dpad_up, dpad_down]
  let dpad_buttons = [];
  ...
  function onKeyDown(e) {
    ...
    var dir = keys_to_directions[e.which];
    // If dir is a valid direction
    if (dir >= 0) {
      held_directions[dir] = 1;
    }
	}
  ...
  const placeCharacter = () => {
    ...
    var input_x = held_directions[directions.right] - held_directions[directions.left];
    var input_y = held_directions[directions.down] - held_directions[directions.up];

    var acc_x = 0;
    var acc_y = 0;
    [acc_x, acc_y] = normalize(input_x, input_y);

    // console.log("accx ", acc_x, " y ", acc_y);

    is_walking = (acc_x != 0 || acc_y != 0) ? "true" : "false";
    
    vel_x *= .88;
    vel_y *= .88;
    vel_x += acc_x * speed * sprinting_factor;
    vel_y += acc_y * speed * sprinting_factor;
    x += vel_x;
    y += vel_y;
    
    for (var dir_idx = 0; dir_idx < 4; ++dir_idx) {
        var is_held = held_directions[dir_idx] == 1;
        if (dir_idx == directions.right && is_held)
        {
          facing_dir = "right";
          break;
        }
        if (dir_idx == directions.left && is_held)
        {
          facing_dir = "left";
          break;
        }
        if (dir_idx == directions.up && is_held)
        {
          facing_dir = "up";
          break;
        }
        if (dir_idx == directions.down && is_held)
        {
          facing_dir = "down";
          break;
        }
    ...
    var right_limit = 352;
    var top_limit = 32;
    var bottom_limit = (16 * 6);
    if (x < cam_x - left_limit) { cam_x = x + left_limit; }
    else if (x > cam_x + right_limit) { cam_x = x - right_limit; }
    if (y < cam_y - top_limit) { cam_y = y + top_limit; }
    else if (y > cam_y + bottom_limit) { cam_y = y - bottom_limit; }
    ...
    map.style.transform = `translate3d( ${-cam_x*pixelSize+camera_left}px, ${-cam_y*pixelSize+camera_top}px, 0 )`;
...
<div class="frame">
  ...
  <div class="camera">
     <div class="map pixel-art" bind:this={map}>
        <div class="character" bind:this={character} facing={facing_dir} walking={is_walking}>
           <div class="shadow pixel-art"></div>
           <div class="character_spritesheet pixel-art"></div>
        </div>
        <h1>USE WASD TO MOVE AND SHIFT TO SPRINT</h1>
        <PythonTutorial />
     </div>
     ...
     <div class="dpad">
        <div class="DemoDirectionUI flex-center">
              <button class="dpad-button dpad-left" bind:this={dpad_left}>
              ...
              <button class="dpad-button dpad-up" bind:this={dpad_up}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 13 13" shape-rendering="crispEdges">
                      <path class="Arrow_outline-top"  stroke="#5f5f5f" d="M1 0h11M0 1h1M12 1h1M0 2h1M12 2h1M0 3h1M12 3h1M0 4h1M12 4h1M0 5h1M12 5h1M0 6h1M12 6h1M0 7h1M12 7h1M0 8h1M12 8h1" />
              ...
              <button class="dpad-button dpad-down" bind:this={dpad_down}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 13 13" shape-rendering="crispEdges">
                      <path class="Arrow_outline-top" stroke="#5f5f5f" d="M1 0h11M0 1h1M12 1h1M0 2h1M12 2h1M0 3h1M12 3h1M0 4h1M12 4h1M0 5h1M12 5h1M0 6h1M12 6h1M0 7h1M12 7h1M0 8h1M12 8h1" />
              ...
              <button class="dpad-button dpad-right" bind:this={dpad_right}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 13 13" shape-rendering="crispEdges">
                      <path class="Arrow_outline-top"  stroke="#5f5f5f" d="M1 0h11M0 1h1M12 1h1M0 2h1M12 2h1M0 3h1M12 3h1M0 4h1M12 4h1M0 5h1M12 5h1M0 6h1M12 6h1M0 7h1M12 7h1M0 8h1M12 8h1" />
